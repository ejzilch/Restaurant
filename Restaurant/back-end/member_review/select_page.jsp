...
<body>
	...
	<div class="wrapper">
		...
		<div id="content">
			...
			<table id="table-1">
				<tr>
					<td>
						<h3 style="margin-bottom: 0;">查詢所有會員評價</h3>
					</td>
				</tr>
			</table>
			<br>
			<ul>
				<%-- listAllMember_Review.jsp --%>
				<li><a
					href='<%=request.getContextPath()%>/back-end/member_review/listAllMember_Review.jsp'
					style="color: #dea554; font-weight: 600;"
					onMouseOver="this.style.color='#ffbc5e';"
					onMouseOut="this.style.color='#dea554';">顯示所有會員評價</a><br> <br></li>
			</ul>

			<%-- 查詢評價 --%>
			<table id="table-1">
				<tr>
					<td><h3 style="margin-bottom: 0;">動態查詢會員評價</h3></td>
				</tr>
			</table>
			<br>
			<%-- 錯誤表列 --%>
			...
			<ul>
				<li>
					<FORM METHOD="post"
						...
						<b>請輸入評價編號:</b> <input type="text" name="review_no"> <input
				...
				<li>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/back-end/member_review/forwarded">
						<b>請選擇評價編號：</b> <select size="1" name="review_no">
				...
				<li>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/back-end/member_review/forwarded">
						<b>請選擇訂餐編號：</b> <select size="1" name="review_no">
							<c:forEach var="member_reviewVO" items="${member_reviewSvc.all}">
								<option value="${member_reviewVO.review_no}">${member_reviewVO.meal_order_no}
							</c:forEach>
						</select> <input type="hidden" name="action" value="getOneForDisplay">
						<input type="submit" value="查詢"> <br> <br>
					</FORM>
				...
				<li>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/back-end/member_review/forwarded">
						<b>請選擇會員評價內容:</b> <select size="1" name="review_no">
							<c:forEach var="member_reviewVO" items="${member_reviewSvc.all}">
				...
				<li>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/back-end/member_review/forwarded">
						<b>請選擇會員評價日期:</b> <select size="1" name="review_no">
							<c:forEach var="member_reviewVO" items="${member_reviewSvc.all}">
								<option value="${member_reviewVO.review_no}">${member_reviewVO.review_date}
							</c:forEach>
						</select> <input type="hidden" name="action" value="getOneForDisplay">
						<input type="submit" value="查詢">
					</FORM>
			...
			<table id="table-1">
				<tr>
					<td>
						<h3 style="margin-bottom: 0;">評價檢舉管理</h3>
					</td>
				</tr>
			</table>
			<br>
			<ul>
				<li><a
					href="<%=request.getContextPath()%>/back-end/report_appraise/listAllReport_Appraise.jsp"
					style="color: #dea554; font-weight: 600;"
					onMouseOver="this.style.color='#ffbc5e';"
					onMouseOut="this.style.color='#dea554';">會員評價檢舉紀錄</a></li>
			</ul>
			<br>
			</p>
		</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			var dateFormat = "mm/dd/yy", from = $("#is_date_startDate")
					.datepicker({
						defaultDate : "+1w",
						changeMonth : true,
						numberOfMonths : 1
